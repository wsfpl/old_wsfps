$(document).ready(function(){
    ...
    $(".burger-menu").click(function(){
     ...
     alert("inside rm download report");
    	  var fromDate = $(".datepickerFrom").val();
          var toDate = $(".datepickerTo").val();
        window.location.href="downloadrpt?fromDate="+fromDate+"&toDate="+toDate;
    })
   ...
   /* $(".view-statusbilling-btn").click(function(){
    ...
    $(document).on("click",".o-display-wrapper",function() {     
	  	    ...
	  	    success: function(data) { 
	  	    	...
	  	    	for (var key in data) {
	  	    		...
	  	    		if(data[key].flag=="C1" || data[key].flag=="C2" || data[key].flag=="C" || data[key].flag=="C21")
	  	    			...
	  	    			innerHtmlDtlPart = innerHtmlDtlPart +'<div class="status-bar"><div class="col-xs-12"><div class="o-product-filtering"><div class="col-xs-10">'
	  ...
	  var innerHtml	= innerHtmlHdrPart + innerHtmlDtlPart + '</div></div><div class="o-all-details-header o-footer"><div class="col-sm-12 p-0"><div class="o-footertot-panel"><div class="col-xs-6 "><p class="">'
  ...
  //  	$(document).on("click",".vieworders",function() {
        ...
        $.ajax({
    	    ...
    	    success: function(data) { 
    	    	...
    	    	var innerHtml = '<div class="modal-header"><button type="button" class="close" data-dismiss="modal">&times;</button><h4 class="modal-title">Shree Traders</h4><div class="v-other-details">'
    	    	    ...
    	    	    $('#modalOrderDtlsId').empty();	
    	    	    $('#modalOrderDtlsId').append(innerHtml);
    	    	    $("#viewdetail-modal").modal('show');
   ...
   //$(".pendingorderviewclass").trigger('click');
    ...
    $('#orderstatus-table-dlr').DataTable({
        "ordering": false,
         "info":     false,
         responsive: true
    });
    ...
    $("#orderstatus-table_wrapper").prepend('<div class="inline-filters"><form class="form-inline"><div class="form-group status-filter"><label class="status">Status:</label><select class="form-control statusSel-cls" id="statusSel"><option value="Approved by TM">Processed by TM</option><option value="Processed by RM">Processed by RM</option></select></div><div class="form-group status-filter"><label class="date">From Date:</label><input type="text" class="datepickerFrom form-control"></input><div class="datepicker-icon"><i class="fa fa-calendar-o fa-calendar-from"></i></div></div><div class="form-group status-filter"><label class="date">To Date:</label><input type="text" class="datepickerTo form-control"></input><div class="datepicker-icon"><i class="fa fa-calendar-o fa-calendar-to"></i></div></div><div class="form-group status-filter"><button type="button" class="btn excelbtn">Excel</button></div></form></div>')
   ...
   // $("#statusSel-s").change(function () { statusSel-s-cls
	...
	$(document).on("change","#pendingorders-select",function() {
		...
		getDlrCards(selected,loginId,loginBussId);
		/*$.ajax({
		    type: "GET",
		    url: 'getdealerDtlsByIdrm',
...
function getDlrCards(selected,loginId,loginBussId){
...
function addCrads(data){
	...
	for ( var key in data) {
		 innerHtml = innerHtml+'<div class="col-lg-4 col-md-6 col-sm-6 col-xs-6">'
			...
			+'  <a  type="button" class="btn btn-view-apre view-page" href="/Dairy/rmpendingordsdtls?dlrNo='+data[key].dlrNo+'">View</a>'
			+'       <div class="pull-right mb-15 approve-reject-box">  <a type="button" class="btn approve-btn btn-view-apre btn-view-approve">Approve</a>  <a type="button" class="btn reject-btn btn-view-apre btn-view-reject">Reject</a></div>'
			+'      </div>'
			+'  </div>'
			+' </div>'
			+' </div>'
...
function datepickerReload(sel,fromDate,toDate){
	  ...
	  $( ".datepicker").datepicker({dateFormat: 'yy-mm-dd'})
		...
		$( ".datepickerTo-s").datepicker({
		   ...
		   $(".statusSel-class").val(sel);
		   $("#statusSel").val(sel);
		  
}

$(document).on("click",".btn-view-approve",function() { 
	var dlrCode=$(this).val();
	bulkApprRejAjaxCall(dlrCode,'approvebulkdlrorders');
});
$(document).on("click",".btn-view-reject",function() { 
	var dlrCode=$(this).val();
	bulkApprRejAjaxCall(dlrCode,'rejectbulkdlrorders');
});

function bulkApprRejAjaxCall(dlrCode,callUrl){
	var bussId = $("#loginBussId").val();
	var selected="ALL";
	var loginId = $("#loginId").val();
	$.ajax({
	    type: "GET",
	    url: callUrl,
	    data: {dlrCode: dlrCode, bussId:bussId},
	    success: function(data) { 
	    	console.log(data);
	    	getDlrCards(selected,loginId,bussId);
	    	$('#pendingorders-select option[value="'+dlrCode+'"]').remove(); 
	    	
	    	var innerHtml = '<div class="modal-content"><div class="modal-body"><p class="status-text" >'+data+'</p></div><div class="modal-footer"><div class="text-center"><a type="button" class="btn btn-default ok-btn" data-dismiss="modal"  >OK</a></div></div></div>';
