$(document).ready(function(){
	 ...
	 $(".o-display-wrapper").click(function(){
	        ...
	        var orderDate = $this.find(".class-order-date").text();
					      ...
					      $.ajax({
					  	  ...
					  	  for (var key in data) {
					  		totalQty = parseFloat(totalQty)+ parseFloat(data[key].tmApprQty);
		...
		innerHtmlDtlPart = innerHtmlDtlPart +'<div class="o-product-panel"><div class=""><div class="col-xs-12"><div class="o-product-filtering"><div class="col-xs-1 p-0">'
					      ...
					      +'<label for="">Quantity(Dlr/TM):</label>'
					      +'<p><i class="fa "></i><span class="span-dlr-qty">'+data[key].qty+'/'+data[key].tmApprQty+'</span></p>'
					      ...
					      +'<input type="text" class="form-control" id="quantity" value="'+data[key].tmApprQty+'" placeholder="Enter Quantity">'
	...
	$(document).on("change","#quantity",function() {
        ...
        if(parseFloat(dlrQty) >= parseFloat(apprQty)  && parseFloat(apprQty) >0){
        	var netPrice = parseFloat(price)* parseFloat(apprQty);
        	$(this).parents('.o-quantity-edit').find('.span-netprice').text(netPrice.toFixed(2));
        }else{
        	var dlrQt = dlrQty.split("/");
        	apprQty = dlrQt[1];
				//apprQty= dlrQty;
        }
          $(this).val(apprQty);
	...
	$(".search-input").on("keyup", function() {
	    ...
	    ul = document.getElementById("headers");
     		...
     		type:"GET",
     		 url: 'approveitemsrm',
     		    ...
     		    success: function(data) {
     		    	var innerHtml;
     		    	if(data == "Error")
     		    	    innerHtml = '<div class="modal-content"><div class="modal-body"><p class="status-text" >Error in Approving data</p></div><div class="modal-footer"><div class="text-center"><a type="button" class="btn btn-default ok-btn" data-dismiss="modal"  onclick="reload()">OK</a></div></div></div>';
     		    	else
     		             innerHtml = '<div class="modal-content"><div class="modal-body"><p class="status-text" >'+data+'</p></div><div class="modal-footer"><div class="text-center"><a type="button" class="btn btn-default ok-btn" data-dismiss="modal"  onclick="reload()">OK</a></div></div></div>';
     		    	/*alert(data);
     		    	var innerHtml;
     		    	if(data == "Error")
     		    	    innerHtml = '<div class="modal-content"><div class="modal-body"><p class="status-text" >Error in Approving data</p></div><div class="modal-footer"><div class="text-center"><a type="button" class="btn btn-default ok-btn" data-dismiss="modal"  onclick="reload()">OK</a></div></div></div>';
     		    	else
     		             innerHtml = '<div class="modal-content"><div class="modal-body"><p class="status-text" >Order Approved Succesfully</p></div><div class="modal-footer"><div class="text-center"><a type="button" class="btn btn-default ok-btn" data-dismiss="modal"  onclick="reload()">OK</a></div></div></div>';*/
	...
	$(document).on("click",".approve-btn-dsk-divice",function() {
         ...
         console.log(appDtls);
         //alert(chkVals);
         $(".overlay").toggleClass('loader-open');
         $.ajax({
     		type:"GET",
     		 url: 'approveitemsrm',
     		    data: {chkVals: chkVals},
		...
		// $('.classOrderId').append('<h5 class="order-left-title">Order Id: '+headerId+' </h5>');
		 ...
		 $.ajax({
		    ...
		    success: function(data) { 
		    	...
		    	var dlrCmt ='';
		    	for ( var key in data) {
		    		innerHtml = innerHtml + '<tr id="apprrej"><td class="text-center"><input type="checkbox" class="chkboxItem" name="" value="'+headerId+'"></td><td>'+data[key].prdNm+'</td><td class="text-center">'+data[key].itemId+'</td><td class="text-center">'+data[key].qty+'/'+data[key].tmApprQty+'</td><td class="text-center"><input type="number" class="qty_class" value="'+data[key].tmApprQty+'" step="0.01"/></td><td class="text-center">'+data[key].unit+'</td><td class="text-center">'+data[key].price+'</td><td class="text-center">'+data[key].netPrice+'</td><td class="text-center">'+data[key].reqDate+'</td></tr>';
		    		counter++;
		    		dlrCmt = data[key].cmt;
		    	  }
